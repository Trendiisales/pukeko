...
import { Facebook, Twitter, Linkedin, Instagram, BarChart2, Grid, Search } from 'lucide-react';
...
const PukekoSocialMedia = () => {
  ...
  const handleSearchAndGenerate = useCallback(() => {
    if (!topic.trim()) return;

    setIsGenerating(true);
    
    // Simulate API call to Google search API
    // In a real implementation, this would connect to a search API
    setTimeout(() => {
      // Simulate search results
      const mockSearchResults = {
        title: `Information about "${topic}"`,
        snippet: `${topic} is trending in various industries. Recent developments show significant impact on markets and consumer behavior. Experts suggest this trend will continue through 2025. Key points include sustainability factors, technological integration, and social impact considerations.`,
        facts: [
          `${topic} has shown 37% growth in the last quarter`,
          `Industry leaders are adopting ${topic} as a core strategy`,
          `Consumer response to ${topic} has been overwhelmingly positive`,
          `${topic} represents a paradigm shift in the industry`
        ]
      };
      
      // Simulate finding images for Instagram
      const mockImages = [
        { url: "/api/placeholder/800/600", alt: `${topic} visual representation` },
        { url: "/api/placeholder/600/600", alt: `${topic} implementation example` }
      ];
      ...
      setPosts({
        facebook: `📢 NEW INSIGHT: ${mockSearchResults.snippet}\n\nKey takeaways:\n• ${mockSearchResults.facts[0]}\n• ${mockSearchResults.facts[1]}\n\nWhat does this mean for your business? Let's discuss in the comments! #${topic.replace(/\s+/g, '')} #TrendAlert`,
        
        twitter: `${topic} is making waves! 🌊\n\n${mockSearchResults.facts[0]}\n\nLearn more about how this affects your strategy. #${topic.replace(/\s+/g, '')} #Innovation`,
        
        linkedin: `📊 Industry Analysis: ${topic}\n\n${mockSearchResults.snippet}\n\nAs professionals, we should consider how ${mockSearchResults.facts[1]}. This presents opportunities for growth and innovation in several key areas.\n\nWhat's your experience with ${topic}? I'd love to hear your thoughts.\n\n#${topic.replace(/\s+/g, '')} #IndustryInsights #ProfessionalDevelopment`,
        
        instagram: `Exploring "${topic}" and its impact 🔍\n\n${mockSearchResults.facts[2]}\n\nSwipe to see how industry leaders are implementing these changes!\n\n#${topic.replace(/\s+/g, '')} #Trending #Innovation #IndustryLeaders #VisualInsights`
      });
    ...
    <>
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg mb-8 border border-green-100">
        <label htmlFor="topicInput" className="block text-sm font-medium text-gray-700 mb-2">
          What would you like to search and post about?
        </label>
        <div className="flex gap-4">
          <div className="flex-grow relative">
            ...
            <textarea
              id="topicInput"
              rows={2}
              className="w-full pl-10 p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors text-gray-900 placeholder-gray-400"
              placeholder="Search for a topic (e.g., 'sustainable packaging')"
              value={topic}
        ...
        {searchResults && (
          <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
            <h3 className="font-medium text-lg mb-2">Search Results</h3>
            <p className="mb-2">{searchResults.snippet}</p>
            <h4 className="font-medium mb-1">Key Facts:</h4>
            <ul className="list-disc pl-5 mb-2">
              {searchResults.facts.map((fact, index) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {platformConfig.map((platform) => (
          <div key={platform.key} className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-all border border-gray-100">
            ...
            {/* Instagram special case for images */}
            {platform.key === 'instagram' && searchImages.length > 0 && (
              <div className="mb-4 flex gap-2 overflow-x-auto py-2">
                {searchImages.map((image, index) => (
                  <div key={index} className="relative min-w-[100px] h-[100px] border border-gray-200 rounded">
                    <img 
                      src={image.url} 
                      alt={image.alt}
